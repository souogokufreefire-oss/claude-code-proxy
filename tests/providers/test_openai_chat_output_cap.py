...
from providers.output_cap import (
    clamp_output_tokens,
...
# --------------------------------------------------------------------------- #
# Transport integration (via GroqProvider, which uses max_tokens on our fork)
# --------------------------------------------------------------------------- #
...
async def test_create_stream_clamps_and_learns_on_cap_rejection(groq_provider):
    ...
    # Our Groq transport keeps max_tokens (SDK + provider compat), so the
    # learned cap will be applied to the same field on retry.
    ...
    assert create.call_args_list[1].kwargs["max_tokens"] == 40960
    assert used_body["max_tokens"] == 40960
...
async def test_learned_cap_clamps_next_request_without_a_400(groq_provider):
    ...
    assert create.call_args.kwargs["max_tokens"] == 40960
    assert used_body["max_tokens"] == 40960
