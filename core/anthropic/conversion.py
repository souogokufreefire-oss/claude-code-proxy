...
def _clean_reasoning_content(value: Any) -> str | None:
    """Normalize reasoning state; empty strings stay explicit replay state."""
...
@dataclass
class _PlainSegment:
    """Converted messages that may be emitted as soon as the head is unblocked."""
...
class _ToolTurnSegment:
    """Assistant ``tool_calls`` plus any post-tool content deferred until results."""
...
class _OpenAIChatHistoryLedger:
    ...
    OpenAI ``chat.completions`` requires every assistant ``tool_calls`` message to be
    followed by its matching ``role: tool`` results before any other message, so later
    transcript messages are buffered until the required tool results are replayed in
    ``tool_calls`` order. Post-tool assistant text stays deferred until its own turn's
    results are emitted.
    """
    ...
    def _emit_tool_turn(self, segment: _ToolTurnSegment) -> None:
        """Emit a tool turn whose results are unavailable (truncated history).

        Missing results are replaced with placeholder ``role: tool`` messages so the
        OpenAI sequence stays valid instead of corrupting the transcript or dropping
        the deferred assistant text.
        """
        if not segment.assistant_emitted:
            self._output.append(segment.assistant_message)
            segment.assistant_emitted = True
        for tool_id in segment.required_tool_ids:
            ...
            if tool_result is not None:
                ...
                self._output.append(_placeholder_tool_result(tool_id))
        self._output.extend(
            AnthropicToOpenAIConverter._deferred_post_tool_to_messages(segment)
        )
...
class AnthropicToOpenAIConverter:
    ...
    def convert_messages(
        ...
        *,
        ...
        return _repair_openai_tool_message_sequence(ledger.finish())
