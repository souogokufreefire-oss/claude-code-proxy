...
def test_convert_mixed_blocks_and_types_and_roles():
    ...
    assert result[3] == {"role": "tool", "tool_call_id": "t1", "content": "result"}
...
# --- Reasoning replay: explicit empty reasoning state ---


def test_convert_assistant_empty_top_level_reasoning_content_is_preserved():
    """Empty ``reasoning_content`` is explicit state, not absence (P2 #1002)."""
...
def test_convert_assistant_empty_reasoning_not_wrapped_in_think_tags():
    """Empty top-level reasoning must not materialize as empty think tags."""
    messages = [
        MockMessage("assistant", "Acknowledged.", reasoning_content=""),
    ]

    result = AnthropicToOpenAIConverter.convert_messages(
        messages, reasoning_replay=ReasoningReplayMode.THINK_TAGS
    )

    assert result == [{"role": "assistant", "content": "Acknowledged."}]


# --- Tool history replay: buffering and ordering ---
...
def test_trailing_tool_use_without_result_replays_placeholder():
    """Local invariant (issue #265 class): truncated history stays valid via placeholder."""
    content = [
        MockBlock(type="tool_use", id="call_trailing", name="Read", input={}),
        MockBlock(type="text", text="After tool"),
    ]
    messages = [MockMessage("assistant", content)]
    result = AnthropicToOpenAIConverter.convert_messages(messages)
    ...
    assert result[0]["tool_calls"][0]["id"] == "call_trailing"
    assert result[1]["tool_call_id"] == "call_trailing"
    assert "Tool result unavailable" in result[1]["content"]
    assert result[2]["content"] == "After tool"
