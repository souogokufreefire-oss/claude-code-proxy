...
def request_model_override(request: Request, settings: Settings) -> str | None:
    """Return a per-request provider/model override from an auth token suffix.

    Claude Code treats ``ANTHROPIC_AUTH_TOKEN`` as stable process state. Letting
    launchers append ``:<model>`` gives users a request-scoped model switch
    without changing the server environment or advertised Claude model ids.
    """
    header = (
        request.headers.get("x-api-key")
        or request.headers.get("authorization")
    ...
    if not suffix:
        return None
    if "/" not in suffix:
        return f"{settings.provider_type}/{suffix}"
    provider = suffix.split("/", 1)[0]
