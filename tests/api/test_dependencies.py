...
def test_request_model_override_uses_full_provider_ref():
    settings = _make_mock_settings()
    request = _request_with_headers(
        {"x-api-key": "freecc:open_router/deepseek/deepseek-r1"}
    )

    assert (
        request_model_override(request, settings) == "open_router/deepseek/deepseek-r1"
    )


def test_request_model_override_prefixes_default_provider_for_raw_model():
    settings = _make_mock_settings(provider_type="nvidia_nim")
    request = _request_with_headers({"authorization": "Bearer freecc:moonshotai/kimi"})

    assert request_model_override(request, settings) == "nvidia_nim/moonshotai/kimi"


def test_request_model_override_returns_none_without_suffix():
    settings = _make_mock_settings(provider_type="nvidia_nim")
