...
def test_model_router_applies_request_override(settings):
    resolved = ModelRouter(
        settings, request_model_override="open_router/deepseek/deepseek-r1"
    ).resolve("claude-3-opus")

    assert resolved.provider_id == "open_router"
    assert resolved.provider_model == "deepseek/deepseek-r1"
    assert resolved.provider_model_ref == "open_router/deepseek/deepseek-r1"
